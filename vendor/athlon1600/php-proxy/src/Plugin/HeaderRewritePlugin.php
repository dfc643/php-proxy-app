...
class HeaderRewritePlugin extends AbstractPlugin {
	...
	function onBeforeRequest(ProxyEvent $event){
		...
		// proxy referer in [urlmode = 0]
		$old_referer = explode("?q=", $event['request']->headers->get('referer'));
		if (count($old_referer) > 1) {
			$new_referer = $old_referer[1];
			if (filter_var($new_referer, FILTER_VALIDATE_URL)) {
				$event['request']->headers->set('referer', $new_referer);
				return;
			}
		}
